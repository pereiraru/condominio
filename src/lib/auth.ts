  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.unitCode = token.unitCode as string | undefined;
        session.user.ownerId = token.ownerId as string | null | undefined;
      }
      return session;
    },
  },
...
// Helper to check if user can access a unit
